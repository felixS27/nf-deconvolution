...
consumption by the `deconwolf` module — `res_x/res_y/res_z` are the pixel
sizes in nm, `dim_z` is the number of z-planes (drives `deconwolf`'s PSF
`--nslice`). Images that are already a single-timepoint, single-channel,
single-scene TIFF are copied through unchanged rather than re-encoded.
Raises an error if the input image is 2D.
